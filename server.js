  ...
  for (const cand of buildChain(cfg, m, withImages)) {
    ...
    try {
      const attempt = await fetch(up.url, {
        ...
        body: JSON.stringify({ model: cand.id, messages, stream: true })
      });
...
async function callModel(cfg, modelId, messages) {
  ...
  for (const cand of uniq) {
    ...
    if (!up) continue;
    try {
      const r = await fetch(up.url, { method: "POST", headers: up.headers, body: JSON.stringify({ model: cand.id, messages, stream: false }) });
